...
class LinkedIn implements LinkedInInterface
{
    ...
    public function api($method, $resource, array $options = [])
    {
        ...
        // Generate an url
        if($resource == '/v2/me' || $resource == 'v2/me' || $resource == 'v2/me?projection=(id,firstName,lastName,profilePicture(displayImage~:playableStreams))' || $resource='/v2/emailAddress?q=members&projection=(elements*(handle~))'){
            unset($options['query']);
